...
export class MaestrosScreenComponent implements OnInit{
  ...
  {value: 6, viewValue: 'Inteligencia Artificial'} // si lo usas también
];
  ...
  this.maestrosService.obtenerListaMaestros().subscribe(
    (response)=>{
      ...
      if(this.lista_maestros.length > 0){
        // Agregar datos del nombre, email y área
        this.lista_maestros.forEach(usuario => {
          usuario.first_name = usuario.user.first_name;
          usuario.last_name = usuario.user.last_name;
          usuario.email = usuario.user.email;

          // Asignar nombre del área a partir del ID
          usuario.area_nombre = this.areas.find(
            area => area.value == usuario.area_investigacion
          )?.viewValue || usuario.area_investigacion;
        });
