    ...
    this.administradoresService.getAdminByID(this.idUser).subscribe(
      (response) => {
        ...
        if (response.user) {
          ...
          console.warn("response.user está vacío para administrador:", response);
        }

        this.user.tipo_usuario = this.rol;
        this.isAdmin = true;
        console.log("Datos administrador:", this.user);
      },
      (error) => {
    ...
    this.maestrosService.getMaestroByID(this.idUser).subscribe(
      (response) => {
        ...
        if (response.user) {
          ...
          console.warn("response.user está vacío para maestro:", response);
        }
    ...
    this.alumnosService.getAlumnoByID(this.idUser).subscribe(
      (response) => {
        ...
        if (response.user) {
          ...
          console.warn("response.user está vacío para alumno:", response);
        }
